...
class GenreControllerTest(@Client("/") val client: HttpClient) {
    ...
    fun testGenreCrudOperations() {
        ...
        var request = HttpRequest.POST("/genres", mapOf("name" to "DevOps"))
        ...
        request = HttpRequest.POST("/genres", mapOf("name" to "Microservices"))
        response = client.toBlocking().exchange(request, Genre::class.java)

        assertEquals(HttpStatus.CREATED, response.status)
        assertEquals("/genres/2", response.header(HttpHeaders.LOCATION))

        var genre = client.toBlocking().retrieve("/genres/2", Genre::class.java)

        assertEquals("Microservices", genre.name)

        val cmdRequest = HttpRequest.PUT("/genres", GenreUpdateCommand(2, "Micro-services"))
